      ...
      <div className="container mx-auto px-4">
          ...
          <div className="relative group order-2 lg:order-1">
            <div className="relative h-64 sm:h-80 md:h-96 lg:h-[500px] rounded-2xl overflow-hidden shadow-xl">
              ...
              <Button 
                variant="secondary" 
                size="sm" 
                className="absolute left-2 md:left-4 top-1/2 -translate-y-1/2 w-10 h-10 md:w-12 md:h-12 rounded-full bg-white/20 backdrop-blur-sm border-white/30 text-white hover:bg-white/30 transition-all duration-200 z-20" 
                onClick={prevImage}
              >
              ...
              <Button 
                variant="secondary" 
                size="sm" 
                className="absolute right-2 md:right-4 top-1/2 -translate-y-1/2 w-10 h-10 md:w-12 md:h-12 rounded-full bg-white/20 backdrop-blur-sm border-white/30 text-white hover:bg-white/30 transition-all duration-200 z-20" 
                onClick={nextImage}
              >
              ...
              <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 opacity-0 group-hover:opacity-100 transition-opacity duration-300 z-20">
                <Button 
                  ...
                  className="w-16 h-16 md:w-20 md:h-20 rounded-full bg-white/20 backdrop-blur-sm border-white/30 text-white hover:bg-white/30 transition-all duration-200"
                >
            ...
            <div className="space-y-4">
              <div className="flex flex-col sm:flex-row gap-3">
                <Button 
                  ...
                  className="flex-1 sm:flex-none bg-gradient-to-r from-emerald-600 to-teal-600 hover:from-emerald-700 hover:to-teal-700 text-white px-6 md:px-8 py-3 rounded-full font-semibold transition-all duration-200 shadow-lg hover:shadow-xl"
                >
                ...
                <Button 
                  ...
                  variant="outline"
                  className="flex-1 sm:flex-none border-emerald-600 text-emerald-600 hover:bg-emerald-50 px-6 md:px-8 py-3 rounded-full font-semibold transition-all duration-200"
                >
                  Lihat Galeri
                </Button>
              </div>
        ...
        <div className="grid grid-cols-3 md:grid-cols-5 gap-2 md:gap-4">
          {showcaseImages.map((image, index) => (
            <div 
              key={index} 
              className="relative h-20 sm:h-24 md:h-32 lg:h-40 rounded-lg md:rounded-xl overflow-hidden cursor-pointer group transition-transform duration-200 hover:scale-105" 
              onClick={() => goToImage(index)}
            >
              <img 
                src={image.url} 
              ...
              <div className={`absolute inset-0 transition-all duration-200 ${
                index === currentImageIndex 
                  ? 'ring-2 md:ring-4 ring-emerald-500 ring-offset-1 md:ring-offset-2' 
                  : 'group-hover:bg-black/20'
              }`}></div>
              
              {/* Image Title Overlay for larger screens */}
              <div className="absolute inset-0 bg-gradient-to-t from-black/60 to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-200 hidden md:flex items-end">
