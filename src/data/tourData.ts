export const tourData = {
  'twa-papandayan': {
    id: 'twa-papandayan',
    title: 'TWA Papandayan',
    description: 'Gunung Papandayan adalah destinasi pendakian ramah pemula yang berlokasi di Garut, Jawa Barat, dengan ketinggian sekitar 2.665 mdpl. Gunung ini dikenal karena jalurnya yang relatif mudah, pemandangan alam yang spektakuler, serta fasilitas yang cocok untuk camping santai, healing trip, maupun edukasi alam.',
    price: 425000,
    duration: 'ONE DAY TRIP',
    location: 'Garut, Jawa Barat',
    category: 'Pegunungan',
    rating: 4.8,
    reviews: 245,
    difficulty: 'Mudah',
    image: '/placeholder.svg',
    destinations: [
      'Kawah Putih',
      'Hutan Mati', 
      'Pondok Saladah'
    ],
    highlights: [
      'Kawah aktif dengan pemandangan spektakuler',
      'Jalur pendakian ramah pemula',
      'Cocok untuk camping dan healing trip',
      'Fasilitas lengkap untuk edukasi alam'
    ],
    itinerary: [
      {
        day: 1,
        title: 'Keberangkatan Jakarta - Papandayan',
        activities: [
          '21.00: Meeting Point Peserta Trip Di Plaza Semanggi',
          '22.00: Keberangkatan Dari Plaza Semanggi',
          '22.30: Keberangkatan Dari RS UKU Cawang',
          '23.00: Keberangkatan dari Hotel Ammarossa Bekasi',
          '23.30: Keberangaktan Dari Pollux Mall Cikarang',
          '23.50: Keberangkatan Dari depan Indomaret Exit Tol Karawang Barat'
        ]
      },
      {
        day: 2,
        title: 'Eksplorasi Gunung Papandayan',
        activities: [
          '02.00: Keberangkatan dari Exit tol cileunyi',
          '04.30: Tiba Di camp papandayan, sarapan dan persiapan kegiatan',
          '06.00: Mulai explore Gunung Papandayan (Kawah, Danau Biru, Hutan Mati, Pondok Saladah)',
          '13.30: Tiba Kembali di Camp Papandayan, Bebersih dan persiapan pulang',
          '15.00: Perjalanan Pulang menuju meeting point awal'
        ]
      }
    ],
    included: [
      'Transportasi AC PP',
      'BBM, TOL Dan Parkir',
      'Souvenir',
      'Makan 1x',
      'Air mineral',
      'Snack',
      'Simaksi',
      'Dokumentasi',
      'Tour leader',
      'Asuransi',
      'P3K Standart'
    ],
    notIncluded: [
      'Makan di luar program',
      'Perbekalan (cemilan/minum) selama trekking Papandayan',
      'Tipping guide'
    ],
    meetingPoints: [
      'Plaza Semanggi (Jakarta)',
      'RS Cawang UKI (Jakarta)',
      'Exit Tol Cileunyi (Bandung)',
      'Depan Hotel Ammorosa (Bekasi)',
      'Indomaret Exit Tol Karawang Barat',
      'Depan Pollux Mall (Cikarang)'
    ],
    departureDates: [
      'JULI: 5, 12, 19, 26',
      'AGUSTUS: 2, 9, 16, 23, 30',
      'SEP: 6, 13, 20, 27'
    ],
    cancellationPolicy: [
      'Trip ini Minimal Keberangkatan 5 Orang Peserta',
      'Biaya trip tergantung dari jumlah peserta',
      'DP (Down Payment) minimal 50% dan pelunasan wajib dibayarkan pada H-3 sebelum keberangkatan',
      'DP tidak dapat dikembalikan jika ada pembatalan dari peserta, tapi bisa dialihkan untuk peserta pengganti',
      'Trip dapat batal atau berubah jadwal sewaktu-waktu apabila terjadi cuaca buruk dan kejadian-kejadian alam tertentu',
      'Wajib menjaga barang-barang pribadi, dan segala kehilangan bukan menjadi tanggung jawab kami sebagai tour operator',
      'Peserta wajib mematuhi aturan yang berlaku di masing-masing destinasi yang dikunjungi',
      'Mohon kearifan peserta untuk ikut serta dalam menjaga lokasi objek wisata dengan tidak melakukan hal-hal yang bersifat merusak'
    ]
  },
  
  'wisata-baduy': {
    id: 'wisata-baduy',
    title: 'Wisuba Baduy',
    description: 'Wisuba Baduy merupakan destinasi wisata budaya yang berlokasi di wilayah Baduy, Kabupaten Lebak, Banten. Tempat ini mengusung konsep wisata edukatif berbasis budaya lokal yang mengangkat kehidupan masyarakat Suku Baduy sebagai inti daya tariknya.',
    price: 250000,
    duration: '2D 1N',
    location: 'Kabupaten Lebak, Banten',
    category: 'Budaya',
    rating: 4.7,
    reviews: 189,
    difficulty: 'Sedang',
    image: '/placeholder.svg',
    destinations: [
      'Baduy Luar',
      'Baduy Dalam'
    ],
    highlights: [
      'Jelajah kampung adat autentik',
      'Upacara budaya simbolik',
      'Workshop kerajinan tradisional',
      'Interaksi langsung dengan warga lokal',
      'Pengalaman hidup tanpa teknologi modern'
    ],
    itinerary: [
      {
        day: 1,
        title: 'Perjalanan ke Baduy Dalam',
        activities: [
          '07.00: Meeting Point Peserta Trip Di Pintu Keluar ST. RangkasBitung',
          '08.00: Keberangkatan Dari ST. Rangkasbitung menuju ke terminal Ciboleger',
          '09.15: Sampai di Terminal Ciboleger & Persiapan Treeking menjuju ke baduy luar',
          '09.30: Treeking menuju baduy luar & jembatan akar',
          '11.30: Sampai di Jembatan Akar & ishoma',
          '12.30: Treeking menuju Baduy Dalam',
          '15.00: Sampai di Homestay Lokal & istirahat',
          '19.00: Makan malam, makrab dan diskusi tentang kehidupan wisuba baduy Dalam'
        ]
      },
      {
        day: 2,
        title: 'Eksplorasi Baduy Dalam & Kembali',
        activities: [
          '05.00: Bangun, sholat dan menikmati Hawa pagi baduy dalam',
          '07.00: Sarapan pagi lalu dipandu untuk melihat suasana perkampungan baduy dalam',
          '10.00: Treeking dari baduy dalam menuju terminal Ciboleger',
          '14.00: Tiba Di terminal Ciboleger, Istirahat dan bersih bersih',
          '15.00: Perjalanan menuju ST. RangkasBitung menggunakan ELF'
        ]
      }
    ],
    included: [
      'Transportasi Lokal PP',
      'Souvenir',
      'Makan 2x',
      'Homestay Lokal',
      'Snack',
      'Simaksi',
      'Dokumentasi',
      'Tour leader',
      'Guide Lokal',
      'Asuransi'
    ],
    notIncluded: [
      'Makan di luar program',
      'Perbekalan (cemilan/minum) selama trekking Baduy',
      'Tipping guide'
    ],
    meetingPoints: [
      'ST RangkasBitung'
    ],
    departureDates: [
      'JUNI: 21, 28',
      'JULI: 5, 12, 19, 26',
      'AGUSTUS: 2, 9, 16, 23, 30'
    ],
    cancellationPolicy: [
      'Trip ini Minimal Keberangkatan 7 Orang Peserta',
      'Biaya trip tergantung dari jumlah peserta',
      'DP (Down Payment) minimal 50% dan pelunasan wajib dibayarkan pada H-3 sebelum keberangkatan',
      'DP tidak dapat dikembalikan jika ada pembatalan dari peserta, tapi bisa dialihkan untuk peserta pengganti',
      'Trip dapat batal atau berubah jadwal sewaktu-waktu apabila terjadi cuaca buruk dan kejadian-kejadian alam tertentu',
      'Wajib menjaga barang-barang pribadi, dan segala kehilangan bukan menjadi tanggung jawab kami sebagai tour operator',
      'Peserta wajib mematuhi aturan yang berlaku di masing-masing destinasi yang dikunjungi',
      'Mohon kearifan peserta untuk ikut serta dalam menjaga lokasi objek wisata dengan tidak melakukan hal-hal yang bersifat merusak'
    ]
  },

  'pulau-harapan': {
    id: 'pulau-harapan',
    title: 'Pulau Harapan',
    description: 'Pulau Harapan adalah kombinasi unik antara destinasi wisata bahari, pusat konservasi lingkungan, dan komunitas lokal yang ramah. Menawarkan kemudahan akses dari Jakarta, suasana alam yang tenang, serta aktivitas edukatif.',
    price: 430000,
    duration: '2D 1N',
    location: 'Kepulauan Seribu, Jakarta',
    category: 'Pantai',
    rating: 4.6,
    reviews: 312,
    difficulty: 'Mudah',
    image: '/placeholder.svg',
    destinations: [
      'Pulau Macan',
      'Pulau Dolphin',
      'Sunset di Pulau Bulat'
    ],
    highlights: [
      'Snorkeling di spot terumbu karang indah',
      'Konservasi penyu dan mangrove',
      'Sunset eksotis di Pulau Bulat',
      'Homestay ramah lingkungan'
    ],
    itinerary: [
      {
        day: 1,
        title: 'Perjalanan & Eksplorasi Pulau',
        activities: [
          '06.00: Peserta berkumpul di Pelabuhan Kali Adem, Muara Angke',
          '07.00: Perjalanan ke Pulau Harapan',
          '11.00: Tiba di Pulau Harapan, check-in Homestay',
          '13.00: Snorkeling di Pulau Macan, wisata ke pulau Dolphin, sunset di Pulau Bulat',
          '17.00: Kembali ke Pulau Harapan untuk mandi dan bersih-bersih',
          '19.00: Makan malam dan bakar ikan'
        ]
      },
      {
        day: 2,
        title: 'Sunrise & Konservasi',
        activities: [
          '05.30: Menikmati Sunrise di Pulau Harapan',
          '07.00: Mandi, coffee break, packing',
          '08.00: Check out homestay',
          '09.00: Explore Taman Mangrove & Penangkaran Penyu',
          '10.30: Tiba di dermaga Pulau Harapan',
          '11.00: Perjalanan menuju pelabuhan kaliadem muara angke'
        ]
      }
    ],
    included: [
      'Kapal Feri PP Pelabuhan Kaliadem – Pulau Harapan',
      'Asuransi penyebrangan',
      'Homestay AC',
      'Makan 3x',
      'Barbeque',
      'Air mineral',
      'Kapal penjelajah pulau',
      'Alat snorkeling',
      'Pelampung',
      'Asuransi',
      'Dokumentasi',
      'Tour Guide'
    ],
    notIncluded: [
      'Tiket penangkaran penyu',
      'Kebutuhan dan obat-obatan pribadi',
      'Tip guide & crew'
    ],
    meetingPoints: [
      'Pelabuhan Kaliadem Muara Angke - Jakarta'
    ],
    departureDates: [
      'JULI: 5-6, 12-13, 19-20, 26-27',
      'AGUSTUS: 2-3, 9-10, 16-17, 23-24, 30-31',
      'SEP: 6-7, 13-14, 20-21, 27-28',
      'OKT: 4-5, 11-12, 18-19, 25-26',
      'NOV: 1-2, 8-9, 15-16, 22-23, 29-30',
      'DES: 3-4, 10-11, 17-18, 24-25, 31-1'
    ],
    cancellationPolicy: [
      'Trip ini Minimal Keberangkatan 7 Orang Peserta',
      'Biaya trip tergantung dari jumlah peserta',
      'DP (Down Payment) minimal 50% dan pelunasan wajib dibayarkan pada H-3 sebelum keberangkatan',
      'DP tidak dapat dikembalikan jika ada pembatalan dari peserta, tapi bisa dialihkan untuk peserta pengganti',
      'Trip dapat batal atau berubah jadwal sewaktu-waktu apabila terjadi cuaca buruk dan kejadian-kejadian alam tertentu',
      'Wajib menjaga barang-barang pribadi, dan segala kehilangan bukan menjadi tanggung jawab kami sebagai tour operator',
      'Peserta wajib mematuhi aturan yang berlaku di masing-masing destinasi yang dikunjungi',
      'Mohon kearifan peserta untuk ikut serta dalam menjaga lokasi objek wisata dengan tidak melakukan hal-hal yang bersifat merusak'
    ]
  },

  'pulau-pramuka': {
    id: 'pulau-pramuka',
    title: 'Pulau Pramuka',
    description: 'Pulau Pramuka adalah pulau berpenduduk dan sekaligus pusat administratif Kabupaten Kepulauan Seribu di utara Jakarta. Luasnya sekitar 16 ha dan dihuni lebih dari 2.000 orang dari etnis Betawi, Bugis, Banten, Jawa, Madura, dan Minangkabau.',
    price: 430000,
    duration: '2D 1N',
    location: 'Kepulauan Seribu, Jakarta',
    category: 'Pantai',
    rating: 4.5,
    reviews: 278,
    difficulty: 'Mudah',
    image: '/placeholder.svg',
    destinations: [
      'Penangkaran penyu & Taman Nasional',
      'Pulau air, pulau gosong petrick dan pulau semak daun',
      'Snorkeling Di spot Soft Coral'
    ],
    highlights: [
      'Penangkaran penyu sisik',
      'Snorkeling di spot coral terbaik',
      'Island hopping 3 pulau cantik',
      'Edukasi konservasi laut'
    ],
    itinerary: [
      {
        day: 1,
        title: 'Perjalanan & Island Hopping',
        activities: [
          '06.00: Peserta berkumpul di Pelabuhan Kali Adem, Muara Angke',
          '07.00: Perjalanan ke Pulau Pramuka',
          '11.00: Tiba di Pulau Pramuka, check-in Homestay',
          '13.00: Snorkeling di spot coral, Wisata ke pulau air, pulau gosong petrick dan pulau semak daun',
          '17.00: Kembali ke Pulau Pramuka untuk mandi dan bersih-bersih',
          '19.00: Makan malam dan bakar ikan'
        ]
      },
      {
        day: 2,
        title: 'Konservasi & Kembali',
        activities: [
          '05.30: Menikmati Sunrise di Pulau Pramuka',
          '07.00: Mandi, coffee break, packing',
          '08.00: Check out homestay',
          '09.00: Explore Taman Nasional & Penangkaran Penyu',
          '10.30: Tiba di dermaga Pulau Pramuka',
          '11.00: Perjalanan menuju pelabuhan kaliadem muara angke'
        ]
      }
    ],
    included: [
      'Kapal Feri PP Pelabuhan Kaliadem – Pulau Pramuka',
      'Asuransi penyebrangan',
      'Homestay AC',
      'Makan 3x',
      'Barbeque',
      'Air mineral',
      'Kapal penjelajah pulau',
      'Alat snorkeling',
      'Pelampung',
      'Asuransi',
      'Dokumentasi',
      'Tour Guide'
    ],
    notIncluded: [
      'Tiket Taman Nasional & penangkaran penyu',
      'Kebutuhan dan obat-obatan pribadi',
      'Tip guide & crew'
    ],
    meetingPoints: [
      'Pelabuhan Kaliadem Muara Angke - Jakarta'
    ],
    departureDates: [
      'JULI: 5-6, 12-13, 19-20, 26-27',
      'AGUSTUS: 2-3, 9-10, 16-17, 23-24, 30-31',
      'SEP: 6-7, 13-14, 20-21, 27-28',
      'OKT: 4-5, 11-12, 18-19, 25-26',
      'NOV: 1-2, 8-9, 15-16, 22-23, 29-30',
      'DES: 3-4, 10-11, 17-18, 24-25, 31-1'
    ],
    cancellationPolicy: [
      'Trip ini Minimal Keberangkatan 7 Orang Peserta',
      'Biaya trip tergantung dari jumlah peserta',
      'DP (Down Payment) minimal 50% dan pelunasan wajib dibayarkan pada H-3 sebelum keberangkatan',
      'DP tidak dapat dikembalikan jika ada pembatalan dari peserta, tapi bisa dialihkan untuk peserta pengganti',
      'Trip dapat batal atau berubah jadwal sewaktu-waktu apabila terjadi cuaca buruk dan kejadian-kejadian alam tertentu',
      'Wajib menjaga barang-barang pribadi, dan segala kehilangan bukan menjadi tanggung jawab kami sebagai tour operator',
      'Peserta wajib mematuhi aturan yang berlaku di masing-masing destinasi yang dikunjungi',
      'Mohon kearifan peserta untuk ikut serta dalam menjaga lokasi objek wisata dengan tidak melakukan hal-hal yang bersifat merusak'
    ]
  },

  'pulau-tidung': {
    id: 'pulau-tidung',
    title: 'Pulau Tidung',
    description: 'Pulau Tidung terdiri dari dua pulau: Tidung Besar (berpenduduk) dan Tidung Kecil (alami), yang dihubungkan oleh ikon wisata legendaris, Jembatan Cinta sepanjang sekitar 800 m.',
    price: 430000,
    duration: '2D 1N',
    location: 'Kepulauan Seribu, Jakarta',
    category: 'Pantai',
    rating: 4.7,
    reviews: 456,
    difficulty: 'Mudah',
    image: '/placeholder.svg',
    destinations: [
      'Snorkeling Di Pulau Tidung Kecil & Besar',
      'Jembatan Cinta',
      'Saung Cemara'
    ],
    highlights: [
      'Jembatan Cinta yang ikonik',
      'Snorkeling di spot terumbu karang',
      'Sunset indah di Saung Cemara',
      'Bersepeda keliling pulau'
    ],
    itinerary: [
      {
        day: 1,
        title: 'Perjalanan & Snorkeling',
        activities: [
          '06.00: Peserta berkumpul di Pelabuhan Kali Adem, Muara Angke',
          '07.00: Perjalanan ke Pulau Tidung',
          '11.00: Tiba di Pulau Tidung, check-in Homestay',
          '13.00: Snorkeling di Pulau Tidung Kecil & Pulau Tidung Besar',
          '16.00: Perjalanan Menuju Saung Cemara',
          '16.30: Explore dan menikmati sunset di saung cemara',
          '17.30: Perjalanan menuju Penginapan',
          '19.00: Makan malam dan bakar ikan'
        ]
      },
      {
        day: 2,
        title: 'Sunrise & Kembali',
        activities: [
          '05.30: Menikmati Sunrise di Pulau Tidung',
          '07.00: Mandi, coffee break, packing',
          '08.00: Check out homestay',
          '09.00: Explore Jembatan Cinta & photo session',
          '10.30: Tiba di dermaga Pulau Tidung',
          '11.00: Perjalanan menuju pelabuhan kaliadem muara angke'
        ]
      }
    ],
    included: [
      'Kapal Feri PP Pelabuhan Kaliadem – Pulau Tidung',
      'Asuransi penyebrangan',
      'Homestay AC',
      'Makan 3x',
      'Barbeque',
      'Air mineral',
      'Kapal penjelajah pulau',
      'Alat snorkeling',
      'Pelampung',
      'Asuransi',
      'Dokumentasi',
      'Tour Guide'
    ],
    notIncluded: [
      'Sewa sepeda',
      'Kebutuhan dan obat-obatan pribadi',
      'Tip guide & crew'
    ],
    meetingPoints: [
      'Pelabuhan Kaliadem Muara Angke - Jakarta'
    ],
    departureDates: [
      'JULI: 5-6, 12-13, 19-20, 26-27',
      'AGUSTUS: 2-3, 9-10, 16-17, 23-24, 30-31',
      'SEP: 6-7, 13-14, 20-21, 27-28',
      'OKT: 4-5, 11-12, 18-19, 25-26',
      'NOV: 1-2, 8-9, 15-16, 22-23, 29-30',
      'DES: 3-4, 10-11, 17-18, 24-25, 31-1'
    ],
    cancellationPolicy: [
      'Trip ini Minimal Keberangkatan 7 Orang Peserta',
      'Biaya trip tergantung dari jumlah peserta',
      'DP (Down Payment) minimal 50% dan pelunasan wajib dibayarkan pada H-3 sebelum keberangkatan',
      'DP tidak dapat dikembalikan jika ada pembatalan dari peserta, tapi bisa dialihkan untuk peserta pengganti',
      'Trip dapat batal atau berubah jadwal sewaktu-waktu apabila terjadi cuaca buruk dan kejadian-kejadian alam tertentu',
      'Wajib menjaga barang-barang pribadi, dan segala kehilangan bukan menjadi tanggung jawab kami sebagai tour operator',
      'Peserta wajib mematuhi aturan yang berlaku di masing-masing destinasi yang dikunjungi',
      'Mohon kearifan peserta untuk ikut serta dalam menjaga lokasi objek wisata dengan tidak melakukan hal-hal yang bersifat merusak'
    ]
  },

  'geopark-ciletuh': {
    id: 'geopark-ciletuh',
    title: 'Geopark Ciletuh',
    description: 'Geopark Ciletuh Palabuhanratu adalah contoh cemerlang perpaduan warisan geologi kuno, keanekaragaman hayati dan budaya, serta keterlibatan komunitas. Dengan status internasional dari UNESCO, geopark ini tidak hanya melestarikan kekayaan alam dan budaya.',
    price: 300000,
    duration: 'ONE DAY TRIP',
    location: 'Sukabumi, Jawa Barat',
    category: 'Alam',
    rating: 4.8,
    reviews: 167,
    difficulty: 'Sedang',
    image: '/placeholder.svg',
    destinations: [
      'Pantai Palampang',
      'Puncak Darma',
      'Curug Cimarunjung',
      'Curug Cikanteh dan Sodong'
    ],
    highlights: [
      'UNESCO Global Geopark',
      'Formasi batuan purba berusia 65 juta tahun',
      'Air terjun spektakuler',
      'Pantai dengan tebing batu eksotis'
    ],
    itinerary: [
      {
        day: 1,
        title: 'Eksplorasi Geopark Ciletuh',
        activities: [
          '02.30 – 05.30: Meeting point',
          '05.40: Perjalanan menuju Geopark Ciletuh, Sukabumi',
          '10.00: Tiba di Puncak Darma, istirahat dan foto-foto',
          '11.00: Perjalanan menuju Curug Cimarunjung',
          '11.30: Explore Curug Cimarunjung',
          '12.30: Perjalanan menuju Curug Cikanteh dan Sodong',
          '13.00: Sampai dan explore Curug Cikanteh dan makan siang',
          '15.30: Explore Curug Sodong dan Cikanteh',
          '17.00: Perjalanan menuju Pantai Palampang',
          '18.30: Sampai di pantai dan foto-foto',
          '20.00: Perjalanan menuju meeting point',
          '23.00: Tiba di meeting point & trip selesai'
        ]
      }
    ],
    included: [
      'Transportasi AC PP',
      'Driver',
      'Tol',
      'Parkir',
      'Tiket Puncak Darma',
      'Tiket Cimarunjung',
      'Tiket Curug Cikanteh dan Sodong',
      'Tiket Pantai Palampang',
      'Dokumentasi',
      'Tour Leader',
      'P3K',
      'Asuransi'
    ],
    notIncluded: [
      'Makan dan pengeluaran pribadi',
      'Kebutuhan & obat-obatan pribadi',
      'Tip guide dan crew'
    ],
    meetingPoints: [
      'Tangerang Selatan – BSD Plaza',
      'Jakarta – Plaza Semanggi',
      'Jakarta – Halte Stasiun Cawang Atas',
      'Bekasi – Hotel Amarossa Bekasi',
      'Bogor – Rest area KM 34 Sentul Selatan'
    ],
    departureDates: [
      'JULI: 6, 19',
      'AGUSTUS: 3, 17',
      'SEP: 6, 14, 27',
      'OKT: 11, 26',
      'NOV: 8, 23',
      'DES: 6, 13, 20, 25, 28'
    ],
    cancellationPolicy: [
      'Trip ini Minimal Keberangkatan 7 Orang Peserta',
      'Biaya trip tergantung dari jumlah peserta',
      'DP (Down Payment) minimal 50% dan pelunasan wajib dibayarkan pada H-3 sebelum keberangkatan',
      'DP tidak dapat dikembalikan jika ada pembatalan dari peserta, tapi bisa dialihkan untuk peserta pengganti',
      'Trip dapat batal atau berubah jadwal sewaktu-waktu apabila terjadi cuaca buruk dan kejadian-kejadian alam tertentu',
      'Wajib menjaga barang-barang pribadi, dan segala kehilangan bukan menjadi tanggung jawab kami sebagai tour operator',
      'Peserta wajib mematuhi aturan yang berlaku di masing-masing destinasi yang dikunjungi',
      'Mohon kearifan peserta untuk ikut serta dalam menjaga lokasi objek wisata dengan tidak melakukan hal-hal yang bersifat merusak'
    ]
  },

  'ujung-genteng': {
    id: 'ujung-genteng',
    title: 'Ujung Genteng',
    description: 'Ujung Genteng adalah kombinasi kekuatan geologi, keindahan alam, konservasi, dan budaya lokal—menjadi "surga tersembunyi" ideal untuk wisata edukatif, petualangan, dan relaksasi yang jauh dari keramaian urban.',
    price: 375000,
    duration: 'ONE DAY TRIP',
    location: 'Sukabumi, Jawa Barat',
    category: 'Pantai',
    rating: 4.6,
    reviews: 203,
    difficulty: 'Mudah',
    image: '/placeholder.svg',
    destinations: [
      'Curug Cikaso',
      'Pantai Karang Gantungan',
      'Penangkaran Penyu',
      'Pantai Ujung Genteng'
    ],
    highlights: [
      'Curug Cikaso yang spektakuler',
      'Pantai dengan formasi karang unik',
      'Konservasi penyu hijau',
      'Destinasi ramah lingkungan'
    ],
    itinerary: [
      {
        day: 1,
        title: 'Berangkat Malam',
        activities: [
          '22.00: Meeting point',
          '23.00: Perjalanan menuju Curug Cikaso'
        ]
      },
      {
        day: 2,
        title: 'Eksplorasi Ujung Genteng',
        activities: [
          '06.00: Tiba di area Curug Cikaso',
          '07.00: Menjelajah Curug Cikaso menggunakan perahu',
          '09.30: Perjalanan menuju Pantai Karang Gantungan',
          '10.00: Explore Pantai Karang Gantungan dan hunting foto',
          '12.00: Perjalanan menuju Penangkaran Penyu',
          '12.30: Explore Penangkaran Penyu dan hunting foto',
          '14.30: Perjalanan menuju Pantai Ujung Genteng',
          '15.00: Explore Pantai Ujung Genteng',
          '17.00: Perjalanan ke tempat oleh-oleh',
          '18.00: Tiba Di Tempat oleh oleh',
          '19.00: Perjalanan Kembali ke meeting point awal',
          '23.00: Tiba di meeting point dan trip selesai'
        ]
      }
    ],
    included: [
      'Transportasi AC PP',
      'Driver',
      'Tol',
      'Parkir',
      'Tiket Curug Cikaso',
      'Tiket Pantai Ujung Genteng',
      'Tiket Pantai Karang Gantungan',
      'Penangkaran Penyu',
      'Dokumentasi',
      'Asuransi',
      'Tour Leader',
      'P3K'
    ],
    notIncluded: [
      'Makan dan pengeluaran pribadi',
      'Kebutuhan & obat-obatan pribadi',
      'Tip guide dan crew'
    ],
    meetingPoints: [
      'Tangerang Selatan – BSD Plaza',
      'Jakarta – Plaza Semanggi',
      'Jakarta – Halte Stasiun Cawang Atas',
      'Bekasi – Hotel Amarossa Bekasi',
      'Bogor – Rest area KM 34 Sentul Selatan'
    ],
    departureDates: [
      'JULI: 12-13, 26-27',
      'AGUSTUS: 9-10, 16-17',
      'SEP: 5-6, 13-14, 27-28',
      'OKT: 11-12, 25-26',
      'NOV: 8-9, 22-23',
      'DES: 6-7, 13-14, 20-21, 27-28'
    ],
    cancellationPolicy: [
      'Trip ini Minimal Keberangkatan 7 Orang Peserta',
      'Biaya trip tergantung dari jumlah peserta',
      'DP (Down Payment) minimal 50% dan pelunasan wajib dibayarkan pada H-3 sebelum keberangkatan',
      'DP tidak dapat dikembalikan jika ada pembatalan dari peserta, tapi bisa dialihkan untuk peserta pengganti',
      'Trip dapat batal atau berubah jadwal sewaktu-waktu apabila terjadi cuaca buruk dan kejadian-kejadian alam tertentu',
      'Wajib menjaga barang-barang pribadi, dan segala kehilangan bukan menjadi tanggung jawab kami sebagai tour operator',
      'Peserta wajib mematuhi aturan yang berlaku di masing-masing destinasi yang dikunjungi',
      'Mohon kearifan peserta untuk ikut serta dalam menjaga lokasi objek wisata dengan tidak melakukan hal-hal yang bersifat merusak'
    ]
  },

  'dieng': {
    id: 'dieng',
    title: 'Dieng',
    description: 'Dieng adalah perpaduan keajaiban alam, kekayaan budaya, dan geologi unik mulai dari udara sejuk, kawah aktif, hingga situs sejarah dan tradisi adat. Kawasan ini cocok untuk keluarga, pecinta alam, fotograf, dan peneliti.',
    price: 775000,
    duration: '3D 2N',
    location: 'Wonosobo, Jawa Tengah',
    category: 'Pegunungan',
    rating: 4.9,
    reviews: 389,
    difficulty: 'Sedang',
    image: '/placeholder.svg',
    destinations: [
      'Tuk Bimalukar',
      'Dieng Plateu Theater',
      'Batu Ratapan Angin',
      'Kawah Sikidang',
      'Candi Arjuna'
    ],
    highlights: [
      'Golden sunrise di Bukit Sikunir',
      'Kawah aktif Sikidang',
      'Kompleks Candi Arjuna',
      'Negeri di atas awan',
      'Budaya anak gimbal'
    ],
    itinerary: [
      {
        day: 1,
        title: 'Keberangkatan ke Dieng',
        activities: [
          '19.00: Peserta berkumpul di meeting point',
          '20.00: Perjalanan menuju Dieng'
        ]
      },
      {
        day: 2,
        title: 'Eksplorasi Dieng',
        activities: [
          '08.00: Tiba di Dieng, sarapan, perjalanan menuju Tuk Bimalukar',
          '09.00: Explore dan hunting foto di Tuk Bimalukar',
          '10.30: Explore Dieng Plateu Theater dan Batu Ratapan Angin',
          '11.30: Check in Homestay, istirahat dan makan siang',
          '14.30: Perjalanan menuju Kawah Sikidang',
          '15.00: Explore Kawah Sikidang',
          '16.30: Explore Candi Arjuna dan menikmati sunset',
          '17.30: Wisata belanja oleh-oleh',
          '18.30: Kembali ke Homestay',
          '19.30: Makan malam dan briefing Sunrise',
          '20.30: Acara Bebas dan Istirahat'
        ]
      },
      {
        day: 3,
        title: 'Sunrise Sikunir & Kembali',
        activities: [
          '02.00: Bangun pagi dan persiapan sunrise ke Bukit Sikunir',
          '02.30: Perjalanan menuju Bukit Sikunir',
          '04.30: Tiba di kawasan dan pengarahan untuk nanjak ke Bukit Sikunir',
          '05.00: Hunting foto, menikmati golden sunrise; negeri di atas awan',
          '07.00: Turun ke bawah & nikmati jajanan Dieng',
          '08.30: Kembali ke penginapan untuk sarapan dan check out',
          '10.00: Check out dan perjalanan menuju Jakarta',
          '22.00: Tiba di Jakarta dan trip selesai'
        ]
      }
    ],
    included: [
      'Transportasi AC PP',
      'Driver, Tol, Parking',
      'Homestay',
      'Makan 4x',
      'Air mineral',
      'Kopi dan teh',
      'Tiket wisata',
      'Perijinan wisata',
      'Asuransi',
      'Dokumentasi',
      'Tour Leader'
    ],
    notIncluded: [
      'Kebutuhan & obat-obatan pribadi',
      'Tip guide dan crew'
    ],
    meetingPoints: [
      'Cawang Dewi Sartika (Sebrang Panasonic), Jakarta'
    ],
    departureDates: [
      'JULI: 4-6, 11-13, 18-20, 25-27',
      'AGUSTUS: 1-3, 8-10, 15-17, 22-24, 29-31',
      'SEP: 5-7, 12-14, 19-21, 26-28',
      'OKT: 3-5, 10-12, 17-19, 24-26, 31-2 NOV',
      'NOV: 7-9, 14-16, 21-23, 28-30',
      'DES: 5-7, 12-14, 19-21, 26-28'
    ],
    cancellationPolicy: [
      'Trip ini Minimal Keberangkatan 7 Orang Peserta',
      'Biaya trip tergantung dari jumlah peserta',
      'DP (Down Payment) minimal 50% dan pelunasan wajib dibayarkan pada H-3 sebelum keberangkatan',
      'DP tidak dapat dikembalikan jika ada pembatalan dari peserta, tapi bisa dialihkan untuk peserta pengganti',
      'Trip dapat batal atau berubah jadwal sewaktu-waktu apabila terjadi cuaca buruk dan kejadian-kejadian alam tertentu',
      'Wajib menjaga barang-barang pribadi, dan segala kehilangan bukan menjadi tanggung jawab kami sebagai tour operator',
      'Peserta wajib mematuhi aturan yang berlaku di masing-masing destinasi yang dikunjungi',
      'Mohon kearifan peserta untuk ikut serta dalam menjaga lokasi objek wisata dengan tidak melakukan hal-hal yang bersifat merusak'
    ]
  },

  'pulau-peucang': {
    id: 'pulau-peucang',
    title: 'Pulau Peucang',
    description: 'Pulau Peucang adalah permata tersembunyi Ujung Kulon: kombinasi pantai karang putih kristal, hutan tropis lebat, dan keanekaragaman satwa liar. Dengan status UNESCO dan konservasi kuat, pulau ini menghadirkan pengalaman alam murni.',
    price: 775000,
    duration: '3D 2N',
    location: 'Taman Nasional Ujung Kulon, Banten',
    category: 'Alam',
    rating: 4.8,
    reviews: 156,
    difficulty: 'Menantang',
    image: '/placeholder.svg',
    destinations: [
      'Pohon Kiara',
      'Karang Copong',
      'Snorkeling Di Pulau Peucang'
    ],
    highlights: [
      'Taman Nasional Ujung Kulon',
      'Wildlife spotting badak jawa',
      'Snorkeling di perairan pristine',
      'Trekking hutan tropis'
    ],
    itinerary: [
      {
        day: 1,
        title: 'Keberangkatan ke Ujung Kulon',
        activities: [
          '20.00: Meeting point di Cawang Dewi Sartika, Jakarta',
          '21.00: Perjalanan menuju dermaga Sumur (istirahat)'
        ]
      },
      {
        day: 2,
        title: 'Eksplorasi Pulau Peucang',
        activities: [
          '07.30: Check-in Penginapan dan acara bebas (main di pantai)',
          '08.30: Explore, hunting dan snorkeling di Pulau Peucang',
          '12.30: Makan siang',
          '13.00: Acara bebas di Pulau Peucang',
          '16.30: Menikmati sunset di Pulau Peucang',
          '18.00: Kembali ke penginapan',
          '20.00: Makan malam dan acara bebas'
        ]
      },
      {
        day: 3,
        title: 'Trekking & Kembali',
        activities: [
          '05.00: Sunrise di dermaga',
          '06.00: Sarapan dan persiapan check out',
          '08.00: Trekking ke Pohon Kiara dan Karang Copong',
          '12.00: Makan siang dan persiapan check out',
          '13.00: Perjalanan menuju dermaga Sumur',
          '15.00: Sampai Dermaga dan persiapan menuju Jakarta',
          '15.30: Perjalanan menuju Jakarta',
          '23.00: Tiba di Jakarta dan trip selesai'
        ]
      }
    ],
    included: [
      'Transportasi AC',
      'Kapal Penjelajah Pulau',
      'Penginapan Barak',
      'Makan 5x',
      'Snack',
      'Air Mineral',
      'Tiket Masuk Wisata',
      'Pelampung',
      'Floties',
      'Tour Guide',
      'Tour Leader',
      'Dokumentasi (GoPro, DSLR, & Drone)',
      'Asuransi'
    ],
    notIncluded: [
      'Kebutuhan dan obat-obatan pribadi',
      'Tip guide dan crew',
      'Canoing Rp 50.000/orang',
      'Alat Snorkeling Rp 50.000/set'
    ],
    meetingPoints: [
      'Cawang Dewi Sartika (Sebrang Panasonic), Jakarta'
    ],
    departureDates: [
      'JULI: 4-6, 11-13, 18-20, 25-27',
      'AGUSTUS: 1-3, 8-10, 15-17, 22-24, 29-31',
      'SEP: 5-7, 12-14, 19-21, 26-28',
      'OKT: 3-5, 10-12, 17-19, 24-26, 31-2 NOV',
      'NOV: 7-9, 14-16, 21-23, 28-30',
      'DES: 5-7, 12-14, 19-21, 26-28'
    ],
    cancellationPolicy: [
      'Trip ini Minimal Keberangkatan 7 Orang Peserta',
      'Biaya trip tergantung dari jumlah peserta',
      'DP (Down Payment) minimal 50% dan pelunasan wajib dibayarkan pada H-3 sebelum keberangkatan',
      'DP tidak dapat dikembalikan jika ada pembatalan dari peserta, tapi bisa dialihkan untuk peserta pengganti',
      'Trip dapat batal atau berubah jadwal sewaktu-waktu apabila terjadi cuaca buruk dan kejadian-kejadian alam tertentu',
      'Wajib menjaga barang-barang pribadi, dan segala kehilangan bukan menjadi tanggung jawab kami sebagai tour operator',
      'Peserta wajib mematuhi aturan yang berlaku di masing-masing destinasi yang dikunjungi',
      'Mohon kearifan peserta untuk ikut serta dalam menjaga lokasi objek wisata dengan tidak melakukan hal-hal yang bersifat merusak'
    ]
  },

  'pulau-sebesi': {
    id: 'pulau-sebesi',
    title: 'Pulau Sebesi',
    description: 'Pulau Sebesi adalah surga alam tersembunyi di selatan Sumatra menggabungkan panorama gunung-karang, hutan tropis, dan laut biru. Pulau ini memadukan keindahan geologi dan ekologi, sarat akan nilai historis pasca-letusan Krakatau.',
    price: 775000,
    duration: '3D 2N',
    location: 'Lampung Selatan',
    category: 'Alam',
    rating: 4.7,
    reviews: 134,
    difficulty: 'Menantang',
    image: '/placeholder.svg',
    destinations: [
      'Sabaku Kecil',
      'Snorkeling Di Spot Geligi Dan Cemara',
      'Pulau Umang',
      'Anak Gunung Krakatau'
    ],
    highlights: [
      'Melihat Anak Gunung Krakatau',
      'Snorkeling di spot eksotis',
      'Sejarah letusan Krakatau',
      'Pulau dengan biodiversitas tinggi'
    ],
    itinerary: [
      {
        day: 1,
        title: 'Perjalanan ke Lampung',
        activities: [
          '18.30: Peserta berkumpul di meeting point',
          '20.00: Perjalanan menuju Pelabuhan Merak',
          '23.30: Tiba di Pelabuhan Merak'
        ]
      },
      {
        day: 2,
        title: 'Eksplorasi Pulau Sebesi',
        activities: [
          '00.00: Perjalanan menuju Pelabuhan Bakauheni',
          '03.30: Tiba di Pelabuhan Bakauheni',
          '07.00: Tiba di Dermaga, persiapan trip',
          '08.00: Perjalanan menuju Pulau Sebuku Kecil',
          '09.00: Explore Pulau Sebuku Kecil',
          '10.00: Snorkeling di spot Geligi dan Cemara',
          '12.00: Perjalanan menuju Pulau Sebesi',
          '12.30: Check in homestay dan makan siang',
          '15.00: Explore Pulau Umang',
          '17.30: Kembali ke homestay',
          '19.00: Makan malam dan acara bebas'
        ]
      },
      {
        day: 3,
        title: 'Anak Gunung Krakatau & Kembali',
        activities: [
          '04.00: Persiapan tracking',
          '05.00: Perjalanan menuju Anak Gunung Krakatau',
          '07.00: Tracking di Anak Gunung Krakatau',
          '09.00: Kembali ke Pulau Sebesi',
          '12.00: Makan siang dan checkout',
          '13.00: Perjalanan menuju Dermaga Canti',
          '15.00: Perjalanan menuju Pelabuhan Bakauheni',
          '16.30: Perjalanan menuju Pelabuhan Merak',
          '20.00: Tiba di Pelabuhan Merak',
          '23.30: Tiba di meeting point dan trip selesai'
        ]
      }
    ],
    included: [
      'Transportasi AC PP',
      'Kapal Feri PP Merak – Bakauheni',
      'Kapal penjelajah pulau',
      'Homestay',
      'Makan 4x',
      'Air mineral',
      'Tiket masuk wisata',
      'Pelampung',
      'Asuransi',
      'Tour Guide',
      'Tour Leader',
      'Dokumentasi',
      'P3K'
    ],
    notIncluded: [
      'Kebutuhan dan obat-obatan pribadi',
      'Tip guide dan crew',
      'Alat Snorkeling Rp 80.000/set'
    ],
    meetingPoints: [
      'Cawang Dewi Sartika (Sebrang Panasonic), Jakarta'
    ],
    departureDates: [
      'JULI: 25-27',
      'AGUSTUS: 15-17, 29-31',
      'SEP: 5-7, 26-28',
      'OKT: 24-26',
      'NOV: 28-30',
      'DES: 26-28'
    ],
    cancellationPolicy: [
      'Trip ini Minimal Keberangkatan 7 Orang Peserta',
      'Biaya trip tergantung dari jumlah peserta',
      'DP (Down Payment) minimal 50% dan pelunasan wajib dibayarkan pada H-3 sebelum keberangkatan',
      'DP tidak dapat dikembalikan jika ada pembatalan dari peserta, tapi bisa dialihkan untuk peserta pengganti',
      'Trip dapat batal atau berubah jadwal sewaktu-waktu apabila terjadi cuaca buruk dan kejadian-kejadian alam tertentu',
      'Wajib menjaga barang-barang pribadi, dan segala kehilangan bukan menjadi tanggung jawab kami sebagai tour operator',
      'Peserta wajib mematuhi aturan yang berlaku di masing-masing destinasi yang dikunjungi',
      'Mohon kearifan peserta untuk ikut serta dalam menjaga lokasi objek wisata dengan tidak melakukan hal-hal yang bersifat merusak'
    ]
  },

  'pulau-pahawang': {
    id: 'pulau-pahawang',
    title: 'Pulau Pahawang',
    description: 'Terletak di Teluk Lampung, Kecamatan Punduh Pidada, Kabupaten Pesawaran, Provinsi Lampung. Memiliki pantai pasir putih, hutan mangrove seluas ~30 ha, dan perairan toska jernih dengan terumbu karang sehat serta ratusan spesies ikan.',
    price: 775000,
    duration: '3D 2N',
    location: 'Pesawaran, Lampung',
    category: 'Pantai',
    rating: 4.6,
    reviews: 198,
    difficulty: 'Mudah',
    image: '/placeholder.svg',
    destinations: [
      'Pulau Kelagian Kecil',
      'Spot Snorkeling Cukuh Bedil',
      'Taman Nemo',
      'Pasir Timbul Pahawang Kecil'
    ],
    highlights: [
      'Pantai pasir putih eksotis',
      'Snorkeling di Taman Nemo',
      'Pasir timbul yang menawan',
      'Konservasi terumbu karang'
    ],
    itinerary: [
      {
        day: 1,
        title: 'Keberangkatan ke Lampung',
        activities: [
          '19.00: Peserta berkumpul di meeting point'
        ]
      },
      {
        day: 2,
        title: 'Eksplorasi Pulau Pahawang',
        activities: [
          '00.00: Perjalanan menuju Pelabuhan Bakauheni dengan kapal feri',
          '03.00: Perjalanan menuju dermaga Ketapang',
          '06.00: Sarapan dan persiapan trip',
          '08.30: Tiba di Pulau Kelagian Kecil dan snorkeling',
          '09.30: Snorkeling di Cukuh Bedil',
          '10.30: Snorkeling di Taman Nemo',
          '12.00: Check in homestay dan makan siang',
          '15.30: Explore sunset di Pasir Timbul, Pahawang Kecil',
          '17.00: Kembali ke penginapan',
          '18.00: Makan malam',
          '20.00: Barbeque ikan bakar'
        ]
      },
      {
        day: 3,
        title: 'Sunrise & Kembali',
        activities: [
          '05.00: Sunrise di Pulau Pahawang Besar',
          '06.00: Sarapan dan check out',
          '08.00: Snorkeling di transplantasi terumbu karang Gosong Bekri',
          '09.00: Pulau Kelagian Besar dan banana boat (opsional)',
          '10.30: Sampai di Dermaga Ketapang',
          '12.00: Makan siang (exclude)',
          '13.00: Oleh-oleh khas Lampung',
          '16.00: Tiba di Pelabuhan Bakauheni',
          '19.00: Tiba di Pelabuhan Merak',
          '23.30: Tiba di Jakarta dan trip selesai'
        ]
      }
    ],
    included: [
      'Transportasi AC PP',
      'Kapal Feri PP Merak – Bakauheni',
      'Homestay',
      'Makan 4x',
      'Bakar ikan',
      'Kapal penjelajah pulau',
      'Retribusi wisata',
      'Dokumentasi foto',
      'Asuransi',
      'Tour Guide',
      'Tour Leader'
    ],
    notIncluded: [
      'Alat snorkeling: Rp 90.000/set',
      'Kebutuhan & obat-obatan pribadi',
      'Tip guide dan crew'
    ],
    meetingPoints: [
      'Plaza Festival, Jakarta'
    ],
    departureDates: [
      'JULI: 4-6, 11-13, 18-20, 25-27',
      'AGUSTUS: 1-3, 8-10, 15-17, 22-24, 29-31',
      'SEP: 5-7, 12-14, 19-21, 26-28',
      'OKT: 3-5, 10-12, 17-19, 24-26, 31-2 NOV',
      'NOV: 7-9, 14-16, 21-23, 28-30',
      'DES: 5-7, 12-14, 19-21, 26-28'
    ],
    cancellationPolicy: [
      'Trip ini Minimal Keberangkatan 7 Orang Peserta',
      'Biaya trip tergantung dari jumlah peserta',
      'DP (Down Payment) minimal 50% dan pelunasan wajib dibayarkan pada H-3 sebelum keberangkatan',
      'DP tidak dapat dikembalikan jika ada pembatalan dari peserta, tapi bisa dialihkan untuk peserta pengganti',
      'Trip dapat batal atau berubah jadwal sewaktu-waktu apabila terjadi cuaca buruk dan kejadian-kejadian alam tertentu',
      'Wajib menjaga barang-barang pribadi, dan segala kehilangan bukan menjadi tanggung jawab kami sebagai tour operator',
      'Peserta wajib mematuhi aturan yang berlaku di masing-masing destinasi yang dikunjungi',
      'Mohon kearifan peserta untuk ikut serta dalam menjaga lokasi objek wisata dengan tidak melakukan hal-hal yang bersifat merusak'
    ]
  }
};
